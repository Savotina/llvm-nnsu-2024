      ...
      if (tgtInstrInfo) {
        ...
        for (auto BBIter = machFunc.begin(); BBIter != machFunc.end(); ++BBIter) {
          ...
          if (!machBasicBlock.empty()) { 
            for (auto InstrIter = machBasicBlock.begin(); InstrIter != machBasicBlock.end(); ++InstrIter) {
              instrCount++;
            }
          }
        }
...
INITIALIZE_PASS(X86SavotinaInstrCounterPass, "x86-savotina-instr-counter",
                "This pass let to instrCount the X86 machine instructions' number", false, false)